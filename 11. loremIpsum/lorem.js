...
const result_article = document.querySelector(".lorem-text");

lorem_form.addEventListener("submit", function(e){
    e.preventDefault();
    const value = parseInt(amount_input.value);
    const randomNumber = Math.floor(Math.random()*text.length);

    if (isNaN(value) || value <= 0) {
        result_article.innerHTML = `<p class="result">${text[randomNumber]}</p>`
    }
    else {
        let tempText = ``;
        for (let i=0,j=0; i<value; i++) {
            tempText += `<p class="result">${text[j]}</p>`
            j = (text.length + j - 1) % text.length;
        }
        result_article.innerHTML = tempText;
    }
